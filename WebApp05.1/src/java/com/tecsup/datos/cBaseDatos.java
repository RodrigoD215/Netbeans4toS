...
protected String generarCodigoAlumnos(String tabla, String campo) throws SQLException {
   String rpta = "";
   String sql = "select count(*) from " + tabla;
   Connection xcon = this.Conectar();
   Statement st = xcon.createStatement();
...
public void grabarNuevoAlumno( String[] datos ) throws SQLException {
   ...
   String xabr = datos[1];
   String xest = datos[2];
   String sql = "insert into alumnos (codigo, nombre, direccion, email, telefono, celular, sexo, estado) values (?,?,?,?,?,?,?,?) ";
   Connection xcon = this.Conectar();
   PreparedStatement ps = xcon.prepareStatement(sql);
   ps.setString(1, xcod);
   ps.setString(2, xnom);
...
public Vector buscarAlumno( String pCodigo ) throws SQLException {
   String sql  = "select codigo, nombre, direccion, email, telefono, celular, sexo, estado" +
...
public void modificarAlumno( String[] datos ) throws SQLException {
   ...
   String xabr = datos[2];
   String xest = datos[3];
   String sql = "update alumnos set nombre=?, direccion=?, email=?, telefono=?, celular=?, sexo=?, estado=? where codigo=? ";
   Connection xcon = this.Conectar();
   PreparedStatement ps = xcon.prepareStatement(sql);
   ps.setString(1, xnom);
   ps.setString(2, xabr);
   ...
   xcon.close();
    ...
    public void eliminarAlumno( String[] datos ) throws SQLException {
   boolean inicio;
   if ( datos.length <= 0 )
      return;
